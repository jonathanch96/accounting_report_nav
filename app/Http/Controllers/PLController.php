...
class PLController extends \TCG\Voyager\Http\Controllers\Controller
{
	...
	public function download(Request $request){
			...
			foreach ($category as $key => $c) {
				...
				while ($formula!="") {
					...
					if(($pos_plus<$pos_min&&$pos_plus!=-1)||($pos_min==-1&&$pos_plus!=-1)){
						...
						$prev_action = $action;
						$action = substr($formula, $pos_plus,$pos_plus); //nnti di kaliin ke sebelumnya
						...
						$prev_action = $action;
						$action = substr($formula, $pos_min,$pos_min);
					...
					foreach ($p["detail"] as $key => $pdet) {
						...
						if($pdet["id"]==$data_to_search){
							...
							//print($value." ".$prev_action." ".$temp_val. " = ");
							$value+=$temp_val;
							//print($value. " <br> ");
				...
				array_push($temp_category_data_detail, [
					...
					'detail'=>$temp_detail,
					'value'=>$value,
					'percentage'=>round($value/$divide,4,PHP_ROUND_HALF_UP),
				]);
		...
		foreach ($tb as $key => $t) {
			$temp_array['date'] = $t["date"];

			foreach ($category as $key => $c) {
				foreach ($t["detail"] as $key => $det) {
					//kalkulasi total/summarynya
					$formula = $c->formula;
					$counter=0;
					while ($formula!="") {
